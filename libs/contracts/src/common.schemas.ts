...
export const ErrorResponseSchema = z.object({
  error: z
    ...
    .min(1)
    .describe(
      "Error type identifier (e.g., 'VALIDATION_ERROR', 'UNAUTHORIZED')",
    ),
  ...
  statusCode: z
    ...
    .min(100)
    .max(599)
    .describe("HTTP status code matching the response"),
});
...
export const MoneySchema = z
  ...
  .int()
  .min(0)
  .describe("Monetary value in cents (1/100 of currency unit)");
...
export const SignedMoneySchema = z
  ...
  .int()
  .describe(
    "Signed monetary value in cents (negative for outflow, positive for inflow)",
  );
...
export const DateRangeQuerySchema = z.object({
  startDate: z
    ...
    .optional()
    .describe("Inclusive start of date range (ISO 8601)"),
  endDate: z
    .string()
    .datetime({ offset: true })
    .optional()
    .describe("Inclusive end of date range (ISO 8601)"),
});
