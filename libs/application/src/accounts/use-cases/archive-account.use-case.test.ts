...
describe("ArchiveAccountUseCase", () => {
  ...
  beforeEach(() => {
    ...
    clock = new FakeClock();
    useCase = new ArchiveAccountUseCase(repo, clock);
  });

  describe("system account protection", () => {
    it("should throw SystemAccountModificationError when archiving a system account", async () => {
      ...
      // Attempt to archive should throw
      await expect(
        useCase.execute({
          userId: USER_ID,
          id: ACCOUNT_ID,
    ...
    it("should allow archiving a non-system account", async () => {
      ...
      const archived = await repo.findById(USER_ID, ACCOUNT_ID);
