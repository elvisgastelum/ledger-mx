...
import type { TransactionRepository, UserId, TransactionId, TransactionLineId, AccountId, CategoryId, EnvelopeId, TransactionLineTargetId, TransactionType, TransactionLineTargetType } from "@ledger-mx/domain";
import type { CreateTransactionInput, CreateTransactionOutput } from "../transaction.types";
  ...
  async execute(input: CreateTransactionInput): Promise<CreateTransactionOutput> {
    ...
    const lines = input.lines.map((line) => {
      ...
      if (line.targetType === "account") {
        ...
        if (!line.categoryId) {
          throw new Error("categoryId is required when targetType is 'category'");
        }
        targetId = line.categoryId as CategoryId as TransactionLineTargetId;
      } else if (line.targetType === "envelope") {
        if (!line.envelopeId) {
          throw new Error("envelopeId is required when targetType is 'envelope'");
        }
    ...
    const transaction = new Transaction({
      id: transactionId,
      userId: userId,
      type: input.type as "income" | "expense" | "transfer" | "adjustment" | "reversal" | "debt_payment",
    ...
    const totalAmountCents = Math.abs(lines.reduce(
