...
import type { FundEnvelopeInput } from "../envelope.types";
import type { IdGenerator, Clock } from "@ledger-mx/application";

export class FundEnvelopeUseCase {
  constructor(
  ...
  async execute(input: FundEnvelopeInput): Promise<Envelope> {
    ...
    // Create transaction (validates sum to zero)
    const transaction = new Transaction({
      id: transactionId,
      userId,
      type: "envelope_allocation",
      occurredAt: now,
      description: `Fund envelope: ${envelope.name}`,
      lines: [line1, line2],
    });
